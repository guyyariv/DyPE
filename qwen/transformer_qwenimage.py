...
# Helper functions for DyPE (from Flux implementation)
...
class QwenEmbedRope(nn.Module):
    def __init__(self, theta: int, axes_dim: List[int], scale_rope=False, dype=False):
        super().__init__()
        self.theta = theta
        self.axes_dim = axes_dim
        self.scale_rope = scale_rope
        self.dype = dype
        self.current_timestep = 1.0  # Default to pure noise

        self.base_resolution = 1328
        self.patch_size = 16
    ...
    def rope_params(self, index, dim, theta=10000, device=None):
        """
        Args:
        ...
        # Ensure index is a tensor on the correct device
        if not isinstance(index, torch.Tensor):
            index = torch.tensor(index, device=device)
        else:
            index = index.to(device)

        # Create theta tensor on the same device
    ...
    def rope_params_yarn(self, index, dim, theta, current_patches, device):
        """
        Apply YARN interpolation with optional DyPE to rope parameters.
        """
        if not self.dype and current_patches <= self.base_patches:
            # No YARN needed, use base RoPE
            return self.rope_params(index, dim, theta, device)
        ...
        # Calculate different frequency bases
        theta_tensor = torch.tensor(theta, dtype=torch.float32, device=device)
        arange_tensor = torch.arange(0, dim, 2, dtype=torch.float32, device=device)
        ...
        # Linear interpolation frequencies
        ...
        freqs_mask = (1 - linear_ramp_mask(low, high, dim // 2, device=device))
        ...
        # Apply mscale if scale > 1 (YARN scaling correction)
        if scale > 1:
    ...
    def _compute_video_freqs(self, frame: int, height: int, width: int, idx: int = 0, device=None) -> torch.Tensor:
        if device is None:
            device = self.pos_freqs.device

        seq_lens = frame * height * width

        # Check if we need YARN interpolation
        current_patches_h = height
        current_patches_w = width
        max_current_patches = max(current_patches_h, current_patches_w)

        if self.dype and max_current_patches > self.base_patches:
            # Use YARN with DyPE for each dimension

            # Frame dimension (no YARN needed usually, but keep for consistency)
            frame_index = torch.arange(idx, idx + frame, device=device)
            freqs_frame = self.rope_params(frame_index, self.axes_dim[0], self.theta, device)
            freqs_frame = freqs_frame.view(frame, 1, 1, -1).expand(frame, height, width, -1)

            # Height dimension with YARN
            if self.scale_rope:
                height_index = torch.cat([
                    torch.arange(-(height - height // 2), 0, device=device),
                    torch.arange(0, height // 2, device=device)
                ])
            else:
                height_index = torch.arange(0, height, device=device)

            freqs_height = self.rope_params_yarn(
                height_index, self.axes_dim[1], self.theta, current_patches_h, device
            )
            freqs_height = freqs_height.view(1, height, 1, -1).expand(frame, height, width, -1)

            # Width dimension with YARN
            if self.scale_rope:
                width_index = torch.cat([
                    torch.arange(-(width - width // 2), 0, device=device),
                    torch.arange(0, width // 2, device=device)
                ])
            else:
                width_index = torch.arange(0, width, device=device)

            freqs_width = self.rope_params_yarn(
                width_index, self.axes_dim[2], self.theta, current_patches_w, device
            )
            freqs_width = freqs_width.view(1, 1, width, -1).expand(frame, height, width, -1)

            freqs = torch.cat([freqs_frame, freqs_height, freqs_width], dim=-1).reshape(seq_lens, -1)
        else:
            # Original implementation without YARN
            # Ensure pos_freqs and neg_freqs are on the correct device
            if self.pos_freqs.device != device:
                self.pos_freqs = self.pos_freqs.to(device)
                self.neg_freqs = self.neg_freqs.to(device)
...
class QwenImageTransformerBlock(nn.Module):
    def __init__(
            self, dim: int, num_attention_heads: int, attention_head_dim: int, qk_norm: str = "rms_norm",
            eps: float = 1e-6
    ):
        super().__init__()
...
class QwenImageTransformer2DModel(
    ...
    """
    The Transformer model introduced in Qwen with DyPE support.
    ...
    Args:
        ...
        dype (`bool`, defaults to `False`):
            Whether to enable DyPE (Dynamic Position Encoding) with timestep-aware scaling.
    """
    ...
    def __init__(
            ...
            dype: bool = False,  # New parameter for DyPE
    ):
        super().__init__()
        ...
        self.pos_embed = QwenEmbedRope(
            ...
            scale_rope=True,
            dype=dype  # Pass DyPE flag to RoPE
        )
    ...
    def forward(
        ...
        """
        The [`QwenTransformer2DModel`] forward method with DyPE support.
        ...
        # Set timestep for DyPE if enabled
        if self.dype and timestep is not None:
            # Normalize timestep to [0, 1] where 1 is pure noise
            # Assuming timestep goes from 0 to 1000 (typical for diffusion models)
            normalized_timestep = timestep.item() / 1000.0 if timestep.numel() == 1 else timestep[0].item() / 1000.0
            # Invert because in diffusion, higher timesteps mean more noise
            normalized_timestep = 1.0 - normalized_timestep
            self.pos_embed.set_timestep(normalized_timestep)
